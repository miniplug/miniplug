...
export default function users() {
  ...
  return function (mp) {
    ...
    // helper for wrapping+adding users
    function join(user) {
      user = wrapUser(user)
      mp._users.push(user)
      return user
    ...
    const getUsers = (...uids) => mp.post('users/bulk', { ids: flatten(uids) }).map(wrapUser)

    // current user & profile
    const saveSettings = partial(mp.put, 'users/settings')
    const setAvatar   = avatar => mp.put('users/avatar',   { id: avatar })
    const setBadge    = badge  => mp.put('users/badge',    { id: badge })
    const setBlurb    = blurb  => mp.put('profile/blurb',  { blurb: blurb })
    const setLanguage = lang   => mp.put('users/language', { language: lang })
